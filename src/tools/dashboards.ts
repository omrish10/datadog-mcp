...
/**
 * Build an absolute dashboard URL.
 *
 * The API reports `url` as a site-relative path ("/dashboard/abc-def/title"),
 * which is not usable on its own, so it is resolved against the configured
 * site. Falls back to constructing the path from the dashboard ID.
 */
function dashboardUrl(
  ...
  if (apiUrl) return apiUrl.startsWith("http") ? apiUrl : appUrl(site, apiUrl);
  return id ? appUrl(site, `dashboard/${id}`) : undefined;
}
...
export function registerDashboardTool(server: McpServer, config: DatadogConfig) {
      ...
      try {
        if (!dashboardId) {
          ...
          const formatted = dashboards.slice(0, 50).map((d) => ({
            ...
            url: dashboardUrl(config.site, d.url, d.id),
        ...
        const formatted = {
          ...
          url: dashboardUrl(config.site, dashboard.url, dashboard.id),
  ...
  server.registerTool(
    "update_dashboard",
    {
      ...
      // Replaces the whole definition: widgets omitted from the list are deleted.
      annotations: {
        readOnlyHint: false,
